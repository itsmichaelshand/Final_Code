...
public class MortgageController {
     ...
     * its called before the initialize() method.
     */
	...
	public void handleCalculate() {
		...
		if (cmbTerm.getValue() == "15 Year") {
			pmtTerm = 15*12; 			// 15 years times 12 monthly payments
		} 
		else if (cmbTerm.getValue() == "30 Year") {
			pmtTerm = 30*12; // 30 years times 12 monthly payments 
		} 
		else {
			pmtTerm = 0;
	...
	public static String setPayment(double income, double expenses, int credit, double house, int pmtTerm) {
		...
		if (r.getInterestRate() == 0){
			...
			pmtString = "Credit score too low";
			return pmtString;
		}
